...
public class ScreenUtil {
    ...
    public static int getScreenH(Context ctx) {
        ...
        if (Build.VERSION.SDK_INT > 13) {
            Point p = new Point();
            ((WindowManager) Objects.requireNonNull(ctx.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getSize(p);
            h = p.y;
        } else {
            h = ((WindowManager) Objects.requireNonNull(ctx.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay()
                    .getHeight();
        }
        return h;
    }
    ...
    public static int getScreenW(Context ctx) {
        ...
        if (Build.VERSION.SDK_INT > 13) {
            ...
            w = ((WindowManager) Objects.requireNonNull(ctx.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay()
                    .getWidth();
        }
        return w;
    }
