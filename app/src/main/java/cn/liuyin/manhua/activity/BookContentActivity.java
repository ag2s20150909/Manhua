    ...
    public void showData(final ContentBean data) {
        ...
        lv.setOnBounceCallBack(new DampingListView.BounceCallBack() {
            ...
            public void onOverScrollUp() {
                if (index > 1) {
                    mHander.obtainMessage(0, "向上翻页").sendToTarget();
                    index--;
                    getData(index);
            ...
            public void onOverScrollDown() {
                if (index < chapters.data.list.size()) {
                    mHander.obtainMessage(0, "向下翻页").sendToTarget();
                    index++;
                    getData(index);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    //预读下一章节
                    if (index < chapters.data.list.size()) {
                        new Thread(new DownloadImgRunnable(BookMaker.getContent(chapters.data.list.get(index).bid, chapters.data.list.get(index).cid), index + 1)).start();
